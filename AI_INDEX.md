...
## 2. 当前关键实现状态（高优先）
- ✅ **取数据层只用 WebSocket API（ws-api）**：`ticker.price` + `klines` + `depth`。
- ✅ **下单层只用 REST API**：真实模式走签名下单。
- ✅ Binance 对接失败策略：**重试 1 次**，仍失败则**通知并抛错**。
- ✅ 指标计算统一使用 **TA-Lib Core**（`com.tictactec.ta.lib.Core`）。
- ✅ 指标已覆盖：MA / RSI / MACD / ATR / BOLL / KDJ / CCI / VWAP / VOLUME。
- ✅ OrderBook 已支持：快照 + 增量事件 + `U/u` 连续性校验 + 自动重拉。
- ✅ 账户/订单同步已支持：listenKey 私有流 + 请求式补偿同步。
- ✅ 私有流可靠性已支持：keepalive、listenKey 失效轮换、重连退避。
- ✅ 回测已支持：HC_ 命名空间隔离 + MA 回放 + 止盈止损 + 超时平仓 + 手续费/滑点/延迟 bar。
- ✅ DuckDB 持久化已覆盖：策略、订单、通知、账户、杠杆。
- ⚠️ 默认 `app.trading.simulation=true`（模拟下单，避免误触发真实交易）。
...
- Binance 数据问题（ws-api）：
  ...
  - `data/OrderBookSubscriptionService.java`
- 指标问题（TA-Lib）：
...
- 私有流同步问题：
  - `service/BinanceUserStreamService.java`
  - `service/ExchangeUserDataSyncService.java`
- 回测问题：
...
## 7. 已知环境限制
- 当前环境下 Maven 访问中央仓库可能返回 403（`spring-boot-starter-parent` 无法拉取），因此不能据此宣称本地编译/测试稳定通过。
